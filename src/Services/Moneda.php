...
class Moneda
{
    ...
    /** HEREDADO */
  public static function letras($xcifra, $moneda="C$"){
    if(  $moneda=="C$" ): $moneda="CORDOBAS"; endif;
    if(  $moneda=="U$" ): $moneda="DOLARES";  endif;
    ...
    $xcifra = str_replace(",", "", $xcifra); // eliminar comas
    $xcifra = str_replace(" ", "", $xcifra); // quito espacios
    $xcifra = str_replace("C", "", $xcifra); // quito C$
    $xcifra = str_replace("$", "", $xcifra); // quito C$
    $xcifra = str_replace("U", "", $xcifra); // quito C$
    $xarray = array(
      0 => "Cero",
      1 => "UN", 
           "DOS",
        ...
        switch ($xy) 
          ...
          case 1: // checa las centenas
            if (substr($xaux, 0, 3) < 100) // si el grupo de tres d�gitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
              ...
              if ($xseek)
                {
                $xsub = self::subfijo($xaux); // devuelve el self::subfijo correspondiente (Mill�n, Millones, Mil o nada)
      ...
      if (trim($xaux) != "")
        ...
        switch ($xz)
          ...
          case 2:
            if ($xcifra < 1 )
              {
              $xcadena = "CERO ".$moneda." CON ".$xdecimales."/100 CENT.";
              }
            if ($xcifra >= 1 && $xcifra < 2)
              {
              $xcadena = "UN ".$moneda." CON ".$xdecimales."/100 CENT.";
              }
            if ($xcifra >= 2)
              {
              $xcadena.= " ".$moneda." CON ".$xdecimales."/100 CENT."; // 
              }
            break;
          } // endswitch ($xz)
        } // ENDIF (trim($xaux) != "")
