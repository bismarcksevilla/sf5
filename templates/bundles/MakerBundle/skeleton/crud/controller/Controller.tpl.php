...
class <?= $class_name ?> extends <?= $parent_class_name; ?><?= "\n" ?>
{
    ...
    <?php if (isset($repository_full_class_name)): ?>
    /**
     * @Route("/listar", name="<?= $route_name ?>_listar",
     *      methods={"GET","POST"},
     *      requirements={
     *          "page"="\d+"
     *      }
     * )
     */
    public function listar($page=1, Request $request, PaginatorInterface $paginator, <?= $repository_class_name ?> $Repo):Response
    {
        ...
        return $this->render('<?= $templates_path ?>/listar.html.twig', [
    ...
    /**
     * @Route("/editar/{<?= $entity_identifier ?>}", name="<?= $route_name ?>_editar",
     *      methods={"GET","POST"},
     *      requirements={
     *          "<?= $entity_identifier ?>"="\d+"
     *      }
     * )
     *  // Usar slug para ocultar id en transacciones delicadas
     *  // "slug"="[a-zA-Z0-9_-]{0,30}"
     */
    public function editar($<?= $entity_identifier ?>=null, Request $request, <?= $repository_class_name ?> $Repo):Response
    {
        $em = $this->getDoctrine()->getManager();
        if($<?= $entity_identifier ?>):
            // $<?= $entity_class_name ?> = $em->getRepository(<?= $entity_class_name ?>::class)->findOneBySlug($slug);
            $<?= $entity_class_name ?> = $em->getRepository(<?= $entity_class_name ?>::class)->findOneBy<?= ucfirst($entity_identifier) ?>($<?= $entity_identifier ?>);
        else:
        ...
        return $this->render('<?= $templates_path ?>/editar.html.twig', [
